...
import { VendorNotesForm } from "@/components/VendorNotesForm";
...
export default async function VendorWorkOrderDetailPage({
  ...
  const workOrder = await prisma.workOrder.findUnique({
    where: { id },
    include: { lineItems: true, uploads: true },
  });

  if (!workOrder) notFound();
  if (workOrder.vendorId !== session!.user.id) redirect("/vendor");

  const total = workOrder.lineItems.reduce((sum, li) => sum + li.price, 0);
  const isLocked = workOrder.status !== "OPEN";
    ...
    <div className="max-w-2xl space-y-6">
      <div>
        <div className="flex items-center justify-between">
          ...
          <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusStyles[workOrder.status]}`}>
        ...
        {workOrder.status === "READY_FOR_OFFICE" && (
          <p className="text-xs text-neutral-500 mt-2">
            This work order has been submitted and is read-only unless your vendor manager
            sends it back to you.
          </p>
        )}
        {workOrder.status === "COMPLETED" && (
          <p className="text-xs text-neutral-500 mt-2">
            This work order is complete. It&rsquo;s read-only unless your vendor manager
            reopens it.
          </p>
        )}
      </div>
      ...
      {isLocked ? (
        ...
        <VendorNotesForm workOrderId={workOrder.id} initialNotes={workOrder.vendorNotes ?? ""} />
      )}
      ...
      {!isLocked && (
        <UploadPanel
          ...
          driveFolderId={workOrder.driveFolderId}
          driveFolderUrl={workOrder.driveFolderUrl}
        />
      )}
      ...
      <MarkReadyButton
        ...
        hasUploads={workOrder.uploads.length > 0}
      />
    </div>
  );
}
